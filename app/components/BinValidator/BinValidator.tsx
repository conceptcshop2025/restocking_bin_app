...
export default function BinValidator({ binLocations }: BinValidatorProps) {
  ...
  return (
    <div className="flex flex-col justify-start items-start p-8 gap-4 flex-wrap bg-neutral-50 rounded-md max-w-[90dvw] overflow-y-auto pt-24">
      ...
      <div className="flex justify-center items-center gap-4">
        <input type="text" name="space-bin" id="space-bin" placeholder="BIN" className="border border-zinc-300 rounded-md px-2 py-4 h-fit" value={validateBinInput} onChange={(e) => setValidateBinInput(e.target.value)} />
        {
